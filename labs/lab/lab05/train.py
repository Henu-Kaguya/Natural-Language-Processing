...
def update_train_state(args, model, train_state):
    """Handle the training state updates with early stopping."""
...
args = Namespace(data_path=os.path.join(BASE_DIR, "data", "conll2003"),
                 vectorizer_file="vectorizer.json",
                 model_state_file="model.pth",
                 save_dir=os.path.join(BASE_DIR, "model_storage", "conll2003", "lstmcrf"),
                 ...
                 glove_filepath=os.path.join(BASE_DIR, 'glove', 'glove.6B.100d.txt'),
                 use_glove=False,
                 dropout=0.5,
                 clip_max_norm=5.0,
                 embedding_dim=100,
...
if args.use_glove and os.path.exists(args.glove_filepath):
    words = vectorizer.token_vocab._token_to_idx.keys()
    embeddings = make_embedding_matrix(glove_filepath=args.glove_filepath,
                                       words=words)
    print("Using pre-trained GloVe embeddings")
else:
    print("Not using pre-trained embeddings (random init)")
    embeddings = None
...
if args.reload_from_files and os.path.exists(args.model_state_file):
    model.load_state_dict(torch.load(args.model_state_file, map_location=args.device))
    print("Reloaded model from checkpoint")
else:
...
for batch_index, batch_dict in enumerate(batch_generator):
    ...
    for sent, true_tag, pred_tag in zip(token_vec, true_tag_vec, pre_tag_vec):
        ...
        result_file.write('\n')
result_file.close()
print(f"Test results written to {args.result_file}")
print("Run 'perl conlleval.pl < result.txt' to evaluate")
