...
class Bigram():
    ...
    #（2）定义函数_forwardSplitSentence，用于获取sentence所有可能的切分方案，是实现本分词算法的核心功能函数之一。在函数中，变量sentence表示待切分文本，word_max_len代表最大词长，split_groups用于存储所有方案，最终要返回切分方案列表。
    ...
    #在函数中，front_word为前向词，word是当前词，返回 P(Wi|Wi-1) 。
    ...
    #在函数中，变量sentence表示待切分句子，word_max_len为最大不切分词长，返回最优切分方案。
    ...
    #在函数中，定义变量sentence来指定出待分词文本，return返回最终的切分序列。
...
#在函数中，定义变量file_name来指定文本路径，is_save表示是否保存，save_file表示保存路径。
...
#在函数中，定义变量file_name来指定文本路径，is_save表示是否保存，save_file表示保存路径，return返回词典。
